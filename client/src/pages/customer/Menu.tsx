...
export default function Menu() {
          ...
          .map((sub) => ({
            ...
            foodItems: sortItems(
              ...
              sub.foodItems.filter((food) => {
                ...
                if (query) {
                  const hay =
                    `${food.name} ${food.description ?? ""} ${tr(food, lang, "name")} ${tr(food, lang, "description")}`.toLowerCase();
  ...
  const flatResults = useMemo(
    () =>
      sortItems(
        sortBy,
        filteredMenu.flatMap((c) => c.subcategories.flatMap((sub) => sub.foodItems))
      ),
    [filteredMenu, sortBy]
  );
        ...
        {menu.length === 0 && !error && (
          <div className="grid gap-3 sm:grid-cols-2" aria-label="Loading menu">
            {Array.from({ length: 4 }).map((_, i) => (
              <div
                key={i}
                className="flex animate-pulse items-center gap-3 rounded-xl border border-slate-200 bg-white p-3"
              >
        ...
        >
          ...
          >
            ...
            {activeItemCount !== null && activeItemCount > 0 && (
              <p className="mt-2 flex items-start gap-2 rounded-lg bg-amber-50 px-3 py-2 text-sm text-amber-800">
                ...
                <span>
                  ...
                  from the kitchen. Anything not yet sent will be cancelled; anything already being prepared stays with
                  staff to serve.
                </span>
              </p>
            )}
            ...
            <p className="mt-1.5 text-sm text-slate-500">
              This frees the table for other guests - you can sign back in from this table's QR code or code anytime.
            </p>
        ...
        >
          ...
          >
            ...
            <div className="mt-3 flex gap-2">
              <Button variant="secondary" className="flex-1" onClick={() => setReviewOpen(false)} disabled={confirming}>
                Add more items
              </Button>
