...
/** Folder names match the ones already in the bucket. Prefixes are created implicitly on first write. */
...
/** Served over the bucket's public URL and safe to link to directly. */
export const PUBLIC_FOLDERS = ["banner", "logo", "product", "team", "awards"] as const;

/** Tenant data - never written to the public bucket, only reachable through authenticated routes. */
...
/** Public images keep their folder in the path; private files are already namespaced by restaurant. */
...
/** Percent-encodes each segment so folder names containing spaces resolve. */
...
export async function getObject(folder: Folder, name: string): Promise<Buffer> {
  const bucket = bucketFor(folder);
  if (bucket) {
    const res = await getR2()!.client.send(
      new GetObjectCommand({ Bucket: bucket, Key: objectKey(folder, name) }),
    );
...
export async function deleteObject(folder: Folder, name: string): Promise<void> {
  ...
  if (bucket) {
    await getR2()!.client.send(
      new DeleteObjectCommand({ Bucket: bucket, Key: objectKey(folder, name) }),
    );
    return;
  }
