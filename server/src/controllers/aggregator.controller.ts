...
async function ingestOrder(
  ...
  const paymentMethod =
    body.paymentMethod === "cash" || body.paymentMethod === "pending" ? body.paymentMethod : "online";
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const unitPrice = round2(typeof line.price === "number" && line.price >= 0 ? line.price : food ? food.price : 0);
    // The aggregator's own instructions ride along on the first line's kitchen note.
    const note =
      i === 0 && instructions ? [instructions, line.note].filter(Boolean).join(" - ") : (line.note || "").trim();
...
export const aggregatorWebhook = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!secret)
    throw new HttpError(503, "Aggregator webhook is not configured. Generate a secret in Restaurant Settings.");
